# User Journey Optimization: Reducing Drop-Offs & Boosting Conversions 
...
- **Revenue Leakage:** An estimated **$2.4M in annualized revenue opportunity** is lost due to funnel friction.  
...
|------------|------------|--------|----------------|
| **1. Account Aggregator (AA) Integration (High Impact)** | Replacing manual PDF upload with an Account Aggregator (API-based bank login) will reduce friction. | Integrate a vendor like Plaid/Yodlee for instant bank linking. | Increase Step 3 conversion from 58% to 70%. |
| **2. Dynamic "Smart" KYC (Technical Fix)** | For Android users on low-end devices, switching from "Video Liveness" to "Passive Still Image" detection will reduce errors. | For Android users on low-end devices, switch from "Video Liveness" to "Passive Still Image" liveness detection (lower bandwidth requirement). | Reduce Android KYC error rate by 50%. |
| **3. Pre-Qualification Slider (Psychological UX)** | Giving users control over loan terms will increase offer acceptance. | Instead of a static "You are approved for $5,000 at 24%," introduce a slider allowing users to trade off Loan Amount for Tenure. | Increase Offer Acceptance Rate by 10%. |

---

### Next Steps  
- **A/B Testing:** Launch experiments in phased cohorts to measure uplift.  
- **Monitoring:** Set up real-time dashboards in Tableau to track KPIs post-implementation.  
- **Iteration:** Continuously refine models and UX based on user feedback and behavioral data.  
